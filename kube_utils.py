...
Duration: {cost_info.get("duration", 0):.2f} seconds"""
    ...
    configmap = client.V1ConfigMap(api_version="v1", kind="ConfigMap", metadata=metadata, data=data)
    ...
    try:
        ...
        try:
            ...
            if e.status == 409:
                print(
                    f"⚠ ConfigMap '{app_name}' already exists in namespace '{namespace}', updating it..."
                )
                updated_configmap = v1.replace_namespaced_config_map(
                    name=app_name, namespace=namespace, body=configmap
                )
